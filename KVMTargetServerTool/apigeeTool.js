...
var password = syncPrompt.prompt({noEchoBack: true});
...
while (1) {
helper.showOptions();
var optionEntered =	helper.inputKVMOptions();
...
if( optionEntered == 1) {
	...
	helper.searchEntry(entry,keyName);
}else if( optionEntered == 2) {
	var updatedEntry = helper.updateKVM(baaSParams.HOST,baaSParams.pathPrefix,baaSParams.orgName,environment,username,password,kvmName);
	if( updatedEntry ) {
		helper.display(updatedEntry);
	}	
	...
	var deletedEntry = helper.deleteKVM(baaSParams.HOST,baaSParams.pathPrefix,baaSParams.orgName,environment,username,password,kvmName);
	if(deletedEntry) {
		displayKVMList(kvmList);
	}
